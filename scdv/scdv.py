...
def get_prob_wordvecs(word2vec_model, document_list):
    ...
    :return: 確率重み付き単語ベクトル
    """
    word_idf_dict = get_idf([" ".join(v) for v in document_list["document"]])
    ...
    prob_wordvecs = get_probability_word_vectors(word2vec_model, word_centroid_map, word_centroid_prob_map,
                                                 num_clusters, word_idf_dict)
...
def get_scdv(prob_wordvecs, document_list):
    ...
    gwbowv = np.zeros((document_list["document"].size, num_clusters * num_features), dtype="float32")
    ...
    for words in document_list["document"]:
        # 文書に含まれる単語を取得
        gwbowv[counter] = create_cluster_vector_and_gwbowv(prob_wordvecs, words, num_features, num_clusters, train=True)
        counter += 1

    percentage = 0.04
    min_no = min_no * 1.0 / len(document_list["document"])
    max_no = max_no * 1.0 / len(document_list["document"])
